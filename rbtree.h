...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        Node<K, V>* z = new Node<K,V>(key,key);
        z->parent = y;
        if (y == nullptr)
        {
            root_ = z;
...
private:
    ...
    void left_rotate(Node<K, V> *x) {
        if (x->right != nullptr)
        {
            Node<K,V>* y = x->right; //set y to be right of x
            x->right = y->left;
            if (y->left != nullptr) 
            {
                y->left->parent = x;
        ...
        {
            std::cout << "Your calling left rotate when you shouldn't" << std::endl;
        }
    }
    /**
    ...
    void right_rotate(Node<K, V> *x) {
        if (x->left != nullptr)
        {
            Node<K,V>* y = x->left; //set y to be right of x
            x->left = y->right;
            if (y->right != nullptr)
            {
                y->right->parent = x;
        ...
        {
            std::cout << "Your calling right rotate when you shouldn't" << std::endl;
        }
    }
    ...
    size_t internal_node_count(Node<K, V> *node) const {
        ...
        if (node->right != nullptr || node->left != nullptr)
        {
            ...
            {
                return internal_node_count(node->left) + 1;
            }
            else if (node->left == nullptr)
            {
                return internal_node_count(node->right) + 1;
            }
            else
            {
    ...
    int diameter(Node<K, V> *node) const {
        ...
        int answer = height(node->left) + height(node->right) + 3;
        answer = std::max(answer,diameter(node->right));
        answer = std::max(answer,diameter(node->left));
        return answer;
    }
    ...
    size_t null_count(Node<K, V> *node) const {
        ...
        else if (node->left != nullptr || node->right != nullptr)
        {
            if (node->right == nullptr)
            {
                return null_count(node->left) + 1;
            }
            else if (node->left == nullptr)
            {
                return null_count(node->right) + 1;
            }
            else
            {
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
        size_t sum = 0;
        for(size_t i=level; i<= size_t(height(node)) + 1; i++) 
        {
            sum += i*width(node, i);
        }
        return sum;
